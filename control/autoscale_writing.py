...
n_char = len(text)
# === get char size based on number of digits ===
char_max_size = 0.12
char_min_size = 0.05
min_num_chars = 1
max_num_chars = 10
...
# char_size = [0.15, 0.15]
# y = mx+b
m = (char_max_size - char_min_size) / (min_num_chars - max_num_chars)
# char_max_size = m*min_num_chars + b
# char_min_size = m*max_num_chars + b
b = char_max_size - m*min_num_chars
y = m*n_char + b
char_size = [y, y]
#     print(char_size)
# raise Exception
...
backup_buffer = np.array([-0.075, 0, 0])

# === get starting point on board based on number of digits ===
# for n_char in range(1, 11):
start_max_x = 0.0
start_min_x = -0.15
# y = mx+b
m2 = (start_max_x - start_min_x) / (min_num_chars - max_num_chars)
b2 = start_max_x - m2*min_num_chars
y2 = m2*n_char + b2
# print(y2)
# raise Exception

# target_pos = np.array([-0.62, -0.15, 0.44])
# target_pos = np.array([-0.62, y2, 0.44])
target_pos = np.array([-0.62, y2, 0.40])
    ...
    for char in text:
        ...
        if char not in text_paths.keys():
            ...
            if char == '9':
                ...
                char_path[0, :] = np.flip(char_path[0, :], axis=0)
                char_path[1, :] = np.flip(char_path[1, :], axis=0)
...
for ii, char in enumerate(text):
    ...
    if ii == 0 and char == '7':
        gauss_path_planner_writing.generate_path(
                state=np.array([
                    pos_path[-1][0], pos_path[-1][1]-0.05, pos_path[-1][2],
                    0, 0, 0,
...
if char == '0':
    ...
    #         autoadjust_av=True
    #     )
    scale1 = np.zeros(int(len(pos_path_to_next_letter)/2))
    scale2 = np.linspace(-0.01, -0.02, int(len(pos_path_to_next_letter)/2))
    scale = np.hstack((scale1, scale2))
    # gauss_path_planner_writing.position_path[:, 0] += scale
    # gauss_path_planner_writing.position_path[:, 0] = np.ones(len(gauss_path_planner_writing.position_path[:, 0])) * -0.62
    # pos_path_to_next_letter[:, 0] = np.ones(len(pos_path_to_next_letter)) * -0.62
    pos_path_to_next_letter[:, 0] += scale
    # import matplotlib.pyplot as plt
    # plt.figure()
    # # plt.plot(scale)
    # plt.plot(pos_path_to_next_letter[:, 0])
    # plt.show()
    # print(gauss_path_planner_writing.position_path.shape)
