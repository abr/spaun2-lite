...
class GaussianPathPlanner():
    ...
    def generate_path(self, state, target, max_v=None, max_a=None, start_v=None, target_v=None, plot=False, autoadjust_av=False):
        """
        ...
        if abs(sum(state[3:6]) + sum(target[3:6])) > 0 or start_v is not None or target_v is not None:
            ...
            #         % (start_v, target_v))
            print(f'Starting at pos: {state[:3]}, ori: {state[6:9]}')
            print(f'Moving to: {target[:3]}, ori: {target[6:9]}')
        ...
        # print(target)
        # print('dist: ', dist)
        # print('starting dist: ', starting_dist)
        # print('ending dist: ', ending_dist)
    ...
    def _plot(self, state, target):
        ...
        print('PLOTTTTTTTTTTTTTING')
        plt.savefig(f"path_{self.position_path[0]}-{self.position_path[-1]}.png")
        plt.show()
